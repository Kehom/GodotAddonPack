...
# The idea here is to use quaternion quantization to compress it using the smallest three
# method, which basically drops the largest component and quantize the rest of them into
# a smaller number of bits. This method only works for rotation quaternions.
...
# the translation has to be taken into account. To keep the separation of "left cube" and
